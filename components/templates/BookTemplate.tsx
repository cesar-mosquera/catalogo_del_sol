          ...
          <div className="notebook-pages">
            ...
            {renderPage(0,
              <div className={`relative flex h-full flex-col overflow-hidden text-white`} style={{ backgroundColor: t.coverBg }}>
                {catalog.coverImage && (
                  <>
                    <Image src={asset(catalog.coverImage)} alt="" fill priority className="object-cover scale-110" sizes="100vw" />
                ...
                <div className="book-cover-content relative z-10 flex h-full flex-col items-center justify-between p-6 pt-10">
                  {/* Top: Badge "Menú Digital" */}
                  <div className="flex flex-col items-center gap-2">
                    <span className="inline-flex items-center gap-1.5 rounded-full border border-white/20 bg-white/10 px-3 py-1 text-[9px] font-bold uppercase tracking-[0.3em] backdrop-blur-sm">
                  ...
                  {/* Center: Logo + Title */}
                  <div className="flex flex-1 flex-col items-center justify-center gap-4 -mt-4">
                    {catalog.logoImage ? (
                      <div className="relative h-28 w-28 drop-shadow-2xl sm:h-36 sm:w-36">
                        <Image src={asset(catalog.logoImage)} alt="Logo" fill priority className="object-contain" sizes="144px" />
                      </div>
                    ) : (
                      <div className="grid h-20 w-20 place-items-center rounded-2xl border border-white/20 bg-white/10 backdrop-blur-sm sm:h-24 sm:w-24">
                        <svg viewBox="0 0 24 24" className="h-8 w-8 fill-current text-white/60" aria-hidden="true">
                    ...
                    <div className="text-center">
                      <h1 className="font-sans text-3xl font-black leading-[0.9] tracking-tight drop-shadow-2xl sm:text-4xl" style={{ color: t.coverTitle }}>
                        {catalog.name}
                      </h1>
                      {catalog.tagline && (
                        <p className="mt-2 text-xs font-medium tracking-wide drop-shadow-lg sm:text-sm" style={{ color: t.coverTagline }}>
                          {catalog.tagline}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.2em] opacity-60">
            ...
            {pages.map((page, si) => renderPage(si + 1 + secOffset,
              ...
              >
                ...
                <div className="flex-1 w-full overflow-hidden flex flex-col gap-2 pt-1">
                  ...
                  <div className={`notebook-back-page flex h-full flex-col items-center justify-center p-8 text-center text-white`} style={{ backgroundColor: t.coverBg }}>
                    ...
                    {back ? (
                      <>
                        {catalog.logoImage && (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={asset(catalog.logoImage)} alt="" className="relative h-16 w-16 object-contain drop-shadow-lg" loading="lazy" decoding="async" />
                        )}
                        {back.title && (
                          <h2 className="relative mt-4 font-sans text-2xl font-bold tracking-tight" style={{ color: t.coverTitle }}>{back.title}</h2>
                        )}
                        {back.subtitle && <p className="relative mt-1 text-xs" style={{ color: t.coverTagline }}>{back.subtitle}</p>}
                        <div className="relative mt-5 w-full max-w-xs space-y-3">
                          {back.rows?.map((row, i) => (
                            <div key={i}>
                              {row.label && (
                                <p className="text-[9px] font-bold uppercase tracking-[0.2em] text-white/50">{row.label}</p>
                              )}
                              <p className="mt-0.5 text-sm font-semibold" style={{ color: t.coverTitle }}>{row.value}</p>
                            </div>
                          ))}
                        </div>
                        {back.footer && (
                          <p className="absolute bottom-4 left-4 right-4 text-[8px] text-white/30">{back.footer}</p>
                        )}
                      </>
                    ) : (
                      <>
                        ...
                        <h2 className="relative mt-4 font-sans text-xl font-bold tracking-tight">Gracias por visitarnos</h2>
                        <p className="relative mt-2 text-xs text-stone-300">Agrega tus favoritos al carrito y confirma por WhatsApp.</p>
                      </>
                    )}
                  </div>
                );
