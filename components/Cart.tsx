...
export function Cart({ catalog }: { catalog: Catalog }) {
  const items = useCart((state) => state.carts[catalog.slug] ?? EMPTY_CART);
  ...
  const send = () => {
    ...
    clear(catalog.slug); setOpen(false);
  };
  return <>
