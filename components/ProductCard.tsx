...
export function ProductCard({ product, compact = false, catalogSlug }: { product: Product; compact?: boolean; catalogSlug: string }) {
  const add = useCart((state) => state.add);
  return <article className={`overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-orange-100 ${compact ? '' : 'md:flex'}`}>
    <Image src={asset(product.image)} alt={product.name} width={640} height={420} sizes={compact ? '100vw' : '(min-width: 768px) 160px, 100vw'} className={compact ? 'h-36 w-full object-cover' : 'h-40 w-full object-cover md:w-40'} />
    <div className="flex flex-1 flex-col gap-2 p-4">
      ...
      <button onClick={() => add(catalogSlug, product)} className="mt-auto rounded-xl bg-orange-600 px-3 py-2 text-sm font-semibold text-white hover:bg-orange-700">Agregar al pedido</button>
    </div>
  </article>;
}
